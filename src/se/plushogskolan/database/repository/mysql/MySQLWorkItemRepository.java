...
public class MySQLWorkItemRepository implements WorkItemRepository {
	...
	public List<WorkItem> getAllByStatus(String status) {
		...
		try (Connection connection = DriverManager.getConnection(devDb, username, password)) {
			...
			while (rs.next()) {
				WorkItem wi = new WorkItem(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5));
				li.add(wi);
			}
			return li;
		} catch (SQLException e) {
	...
	public List<WorkItem> getAllByTeam(String teamId) {
		...
		try (Connection connection = DriverManager.getConnection(devDb, username, password)) {
			...
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				WorkItem wi = new WorkItem(rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7));
				li.add(wi);
			}
			return li;
		} catch (SQLException e) {
	...
	public List<WorkItem> getAllByUser(String userId) {
		...
		try (Connection connection = DriverManager.getConnection(devDb, username, password)) {
			...
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				WorkItem wi = new WorkItem(rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6));
				li.add(wi);
			}
			return li;
		} catch (SQLException e) {
	...
	public WorkItem getById(String id) {
		...
		try (Connection connection = DriverManager.getConnection(devDb, username, password)) {
			...
			while (rs.next()) {
				result = new WorkItem(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5));
			}
